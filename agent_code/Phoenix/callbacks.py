...
def calculate_direction_to_nearest_coin_dijkstra(game_state):
    ...
    priority_queue = [(0, self_x, self_y, [])]  # (cost, x, y, path)
    ...
    while priority_queue:
        ...
        for dx, dy in movements:
            ...
            if 0 <= new_x < len(field) and 0 <= new_y < len(field[0]):
                ...
                if field[new_x][new_y] == 0 and (new_x, new_y) not in visited:
                    visited.add((new_x, new_y))

                    # Calculate the cost of moving to the new position
                    new_cost = cost + 1  # Assuming uniform cost for all movements

                    # Add the new position and path to the priority queue
                    new_path = path + [directions[movements.index((dx, dy))]]
                    heapq.heappush(priority_queue, (new_cost, new_x, new_y, new_path))
...
def act(self, game_state: dict) -> str:
    ...
    if self.train:
        ...
        if random.random() < epsilon:
            ...
            direction_to_coin = calculate_direction_to_nearest_coin_dijkstra(game_state)
            print(direction_to_coin)
